...
$columns = array(
    ...
    array( 'db' => '`STA`.`CATALOG_identifier`', 'dt' => 'STATION_CATALOG_identifier', 'field' => 'STATION_CATALOG_identifier', 'as' => 'STATION_CATALOG_identifier', 'formatter' => function( $d, $row ) {
			$string = $d;
			if ($row[7] != '') $string = $d.'('.$row[7].')';
            return '<div align="center">'.$string.'</div>';
		}
    ),
    array( 'db' => '`ISD`.`date`', 'dt' => 'date', 'field' => 'date', 'as' => 'date', 'formatter' => function( $d, $row ) {
            return '<div align="center">'.$d.'</div>';
		}
    ),
    array( 'db' => '`ISD`.`time`', 'dt' => 'time', 'field' => 'time', 'as' => 'time', 'formatter' => function( $d, $row ) {
            return '<div align="center">'.$d.'</div>';
		}
    ),
    array( 'db' => '`ISD`.`air_temperature`', 'dt' => 'air_temperature', 'field' => 'air_temperature', 'as' => 'air_temperature', 'formatter' => function( $d, $row ) {
            return '<div align="center">'.$d.'</div>';
		}
    ),
    array( 'db' => '`ISD`.`speed_rate`', 'dt' => 'speed_rate', 'field' => 'speed_rate', 'as' => 'speed_rate', 'formatter' => function( $d, $row ) {
            return '<div align="center">'.$d.'</div>';
		}
    ),
