...
// DB table to use
$table = 'precipitation_hourlydata_summary';
$joinQuery = "FROM `{$table}` AS `PSUMMARY` INNER JOIN `precipitation_hourlydata_stations` AS `PSTA` ON (`PSUMMARY`.`Station`=`PSTA`.`COOP`) ";
...
$columns = array(
    ...
    array( 'db' => '`PSTA`.`Elevation`', 'dt' => 'Elevation', 'field' => 'Elevation', 'as' => 'Elevation', 'formatter' => function( $d, $row ) {
        return $d;
    }
    ),
    array( 'db' => '`PSTA`.`Latitude`', 'dt' => 'Latitude', 'field' => 'Latitude', 'as' => 'Latitude', 'formatter' => function( $d, $row ) {
        return $d;
    }
    ),
    array( 'db' => '`PSTA`.`Longitude`', 'dt' => 'Longitude', 'field' => 'Longitude', 'as' => 'Longitude', 'formatter' => function( $d, $row ) {
        return $d;
    }
    ),
